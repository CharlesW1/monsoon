...
class AppWindowViewModel(object):
    ...
    def __init__(
        ...
        # Persistent executor for parallelizing champion data fetching in on_data
    ...
    def _fetch_champion_balance(self, champion_id: int, category: str):
        """Helper to fetch champion balance and icon in parallel."""
        ...
        # Fetch icon (uses DataDragon's internal cache)
    ...
    def on_data(self, data: ChampionSelectSessionModel):
        ...
        # Parallelize fetching of champion data and icons.
        # This significantly improves UI responsiveness when multiple champions are updated at once,
        # especially for the "available" list (bench) which can have up to 10 champions.
        team_ids = data.team_champion_ids or []
        avail_ids = data.available_champion_ids or []

        # Submit all tasks to the persistent executor
    ...
    def __del__(self):
        # Ensure executor is shut down when the viewmodel is destroyed
