...
class AppWindowViewModel(object):
    ...
    def __init__(
        ...
        # Persistent executor for background tasks (e.g. fetching champion data/icons)
        self.executor = concurrent.futures.ThreadPoolExecutor(max_workers=App.EXECUTOR_WORKERS)
    ...
    def _fetch_balance_for_champion_id(self, champion_id: int, category: str):
        """Helper to fetch balance data and icon for a champion id concurrently"""
        ...
        # This call may be network-bound if icon is not cached
    ...
    def on_data(self, data: ChampionSelectSessionModel):
        ...
        # Parallelize data and icon fetching for all champions in session
        # Use separate lists of futures to maintain category indexing
        team_futures = [self.executor.submit(self._fetch_balance_for_champion_id, cid, "team") for cid in team_ids]
        avail_futures = [self.executor.submit(self._fetch_balance_for_champion_id, cid, "available") for cid in avail_ids]
